...
describe('Maximum Charge Bridge', () => {
  ...
  it('should identify residue 36 (Kr/Cm) as having maximum charge', () => {
    const maxBridge = getMaximumChargeBridge();

    // From specification: Residue 36 with C=(7,3,0)
    expect(maxBridge?.residue).toBe(36);
    expect(maxBridge?.charge.c2).toBe(7);
    expect(maxBridge?.charge.c3).toBe(3);
