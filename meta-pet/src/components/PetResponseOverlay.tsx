  ...
  return (
    <>
      ...
      {responseHistory.length > 0 && (
        <div className="fixed bottom-4 right-4 w-64 max-h-48 overflow-y-auto pointer-events-none z-40">
          <div className="space-y-1">
            {responseHistory.slice(0, 5).map((response, index) => (
              <div
                key={response.id}
                className="text-xs bg-slate-900/80 backdrop-blur-sm rounded-lg px-3 py-2 text-white/60 border border-slate-700/50"
