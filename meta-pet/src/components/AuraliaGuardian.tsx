...
// ===== AUDIO SYSTEM =====
const useAuraliaAudio = (enabled: boolean, energy: number, mood: number) => {
...
export function AuraliaGuardian() {
  ...
  const [field] = useState<Field>(() => initField("AURALIA"));
  ...
  const { playNote } = useAuraliaAudio(audioEnabled, vitals.energy, vitals.mood);
  ...
  const computeGenome = (): { red60: number; blue60: number; black60: number; } => {
    if (!genome) return { red60: 50, blue60: 50, black60: 50 };
    const pulseSum = field.pulse.slice(0, 20).reduce((a, b) => a + b, 0);
    const ringSum = field.ring.slice(0, 20).reduce((a, b) => a + b, 0);
    const red60 = Math.min(100, (pulseSum * 1.2 + vitals.energy * 0.7 + (100 - vitals.hygiene) * 0.3) % 100);
    const blue60 = Math.min(100, (ringSum * 1.1 + vitals.mood * 0.6 + 50 * 0.5) % 100);
    const black60 = Math.min(100, ((pulseSum + ringSum) * 0.8 + vitals.energy * 0.4 + 50 * 0.6) % 100);
  ...
  const sigilPoints = useMemo(() => generateSigil("AURALIA"), [generateSigil]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (Math.random() < 0.02 * (vitals.energy / 50)) {
  ...
  const getActiveForm = (): FormKey => {
    if (vitals.energy < 30 && vitals.hygiene < 50) return 'meditation';
    if (vitals.mood > 80 && activatedPoints.size >= 5) return 'celestial';
    if (vitals.energy > 80 && vitals.mood > 70 && activatedPoints.size >= 5) return 'wild';
    if (vitals.energy > 70 && vitals.mood > 60) return 'vigilant';
    if (vitals.mood > 60 && activatedPoints.size >= 3) return 'sage';
    return 'radiant';
  };
  ...
  return (
    <div className="relative w-full h-full">
            ...
            <g>
              ...
              {sigilPoints.map((p, i) => (
                <circle
                  ...
                  fill={currentForm.tealAccent} opacity={0.5}
      ...
      <div className="absolute bottom-2 left-0 right-0 text-center">
        <p key={whisper.key} className="text-xs text-yellow-300/80 italic animate-pulse">
          {whisper.text}
        </p>
      </div>
      ...
      <div className="absolute top-2 left-0 right-0 text-center">
        <p className="text-xs font-semibold" style={{ color: currentForm.primaryGold }}>
          {currentForm.name}
        </p>
      </div>
      ...
      <button
        ...
        className="absolute top-2 right-2 text-xl opacity-60 hover:opacity-100 transition-opacity"
        title="Toggle Audio"
      >
        {audioEnabled ? '🔊' : '🔇'}
      </button>
